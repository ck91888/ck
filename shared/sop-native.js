...
function create(kind){current=null;
 if(kind==='need'){form('新建作业需求',depInput()+input('title','作业名称')+input('customer','客户')+select('source_type','关联来源',{inbound:'入库计划',outbound:'出库计划'})+input('source_id','关联单据（可下拉选择）','text','',false)+area('instructions','操作要求')+input('owner','接单负责人')+input('deadline','要求完成时间','datetime-local','',false)+input('location','货物位置','text','',false)+input('reason','已有需求时的追加原因','text','',false),'sop_need_create',v=>v,()=>load());attachSources('source_id','inbound');$('editor').elements.source_type.onchange=e=>attachSources('source_id',e.target.value);}
...
async function openSource(){
 ...
 await load();
 form(source==='outbound'?'将出库操作统一为关联作业':'从入库计划创建作业需求',depInput()+input('title','作业名称','text','关联作业 '+doc.title)+input('customer','客户','text',doc.customer)+area('instructions','操作要求',doc.instructions)+input('owner','接单负责人')+input('location','货物位置','text','',false)+input('deadline','完成期限','datetime-local','',false)+(options.supplement?input('reason','新增作业原因（说明与原作业的区别）'):''),source==='outbound'?'sop_need_from_outbound':'sop_need_create',v=>({...v,source_id,source_type:source}),r=>detail(r.id));
