...
export function database(file=':memory:'){
 ...
 return {raw:db,prepare,async batch(statements){db.exec('BEGIN');try{const out=[];for(const s of statements){const prepared=db.prepare(s.sql);if(prepared.columns().length){out.push({success:true,results:prepared.all(...s.args),meta:{changes:0}});}else{const r=prepared.run(...s.args);out.push({success:true,results:[],meta:{changes:r.changes}});}}db.exec('COMMIT');return out;}catch(e){db.exec('ROLLBACK');throw e;}}};
}
